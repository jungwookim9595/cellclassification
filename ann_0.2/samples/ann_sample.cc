...
std::istream		*data_in	= NULL;		// input for data points
std::istream		*query_in	= NULL;		// input for query points

ANNbool readPt(std::istream &in, ANNpoint p)		// read point (false on EOF)
{
    for (int i = 0; i < dim; i++) {
...
main(int argc, char **argv)
{
	...
	printPt(std::cout, data_pts[n_pts]);
	n_pts++;
    }
...
{
    static std::ifstream dataStream;			// data file stream
    static std::ifstream queryStream;		// query file stream
