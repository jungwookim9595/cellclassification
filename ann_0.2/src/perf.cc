...
{
    std::cout.precision(4);			// set floating precision
    std::cout << "  (Performance stats: "
	 << " [      mean :    stddev ]<      min ,       max >\n";
    print_one_stat(std::string("    leaf_nodes       ").c_str(), visit_lfs, 1);
    print_one_stat(std::string("    splitting_nodes  ").c_str(), visit_spl, 1);
    print_one_stat(std::string("    shrinking_nodes  ").c_str(), visit_shr, 1);
    print_one_stat(std::string("    total_nodes      ").c_str(), visit_nds, 1);
    print_one_stat(std::string("    points_visited   ").c_str(), visit_pts, 1);
    print_one_stat(std::string("    coord_hits/pt    ").c_str(), coord_hts, N_data_pts);
    print_one_stat(std::string("    floating_ops_(K) ").c_str(), float_ops, 1000);
    if (validate) {
    	print_one_stat("    average_error    ", average_err, 1);
    	print_one_stat("    rank_error       ", rank_err, 1);
    }
    std::cout.precision(6);			// restore the default
    std::cout << "  )\n";
}
