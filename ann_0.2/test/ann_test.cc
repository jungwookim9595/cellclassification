...
void Error(				// error routin
    const char		*msg,		// error message
    ANNerr		level)		// abort afterwards
{
    if (level == ANNabort) {
	std::cerr << "ann_test: ERROR------->" << msg << "<-------------ERROR\n";
	exit(1);
    }
    else {
	std::cerr << "ann_test: WARNING----->" << msg << "<-------------WARNING\n";
    }
}
...
int main()
{
	...
	//----------------------------------------------------------------
	    ...
	    std::cin >> arg;				// input argument
	    if (!strcmp(arg, "on")) {
	    	validate = ANNtrue;
		std::cout << "validate = on   "
		...
		Error(std::string("validate argument must be \"on\" or \"off\"").c_str(), ANNabort);
	    }
	}
	//----------------------------------------------------------------
	//  distribution option
	//----------------------------------------------------------------
	    ...
	    std::cin >> arg;				// input name and translate
	    distr = (Distrib) lookUp(arg, distr_table, N_DISTRIBS);
	    if (distr >= N_DISTRIBS) {		// not something we recognize
		Error(std::string("Unknown distribution level").c_str(), ANNabort);
	    }
	}
	//----------------------------------------------------------------
	//  stats option
	//----------------------------------------------------------------
		...
		Error(std::string("Unknown statistics level").c_str(), ANNabort);
	    }
	    if (stats > SILENT)
	    	std::cout << "stats = " << arg << "\n";
	}
	...
	//----------------------------------------------------------------
		...
		Error(std::string("Unknown shrinking rule").c_str(), ANNabort);
	    }
	}
	//----------------------------------------------------------------
	//  label operation
	...
	//----------------------------------------------------------------
		...
		Error(std::string("Cannot dump.  No tree has been built yet").c_str(), ANNwarn);
	    }
	    else {				// there is a tree
						// try to open file
		std::ofstream out_dump_file(arg);
		if (!out_dump_file)
		    Error(std::string("Cannot open dump file").c_str(), ANNabort);
						// dump the tree and points
		the_tree->Dump(ANNtrue, out_dump_file);
		if (stats > SILENT) {
	    	    std::cout << "(Tree has been dumped to file " << arg << ")\n";
		}
	    }
	}
	//----------------------------------------------------------------
	...
	//----------------------------------------------------------------
		...
		Error(std::string("Search type must be \"standard\" or \"priority\"").c_str(), ANNabort);
	    }
	    if (data_pts == NULL || query_pts == NULL) {
		Error(std::string("Either data set and query set not constructed").c_str(), ANNabort);
	    }
	    if (the_tree == NULL) {
		Error(std::string("No search tree built.").c_str(), ANNabort);
	    }
	    ...
	    if (stats > SILENT) {
	    	...
	    	std::cout << "  dim           = " << dim << "\n";
	    	std::cout << "  search_method = " << arg << "\n";
	    	std::cout << "  epsilon       = " << epsilon << "\n";
	    	std::cout << "  near_neigh    = " << near_neigh << "\n";
		if (max_pts_visit)
		    std::cout << "  max_pts_visit = " << max_pts_visit << "\n";
		if (validate)
	    	    std::cout << "  true_nn       = " << true_nn << "\n";
		...
		if (stats >= EXEC_TIME) {	// print exec time summary
			...
			double(query_time)/(query_size*CLOCKS_PER_SEC)
			 << " sec/query";
		    #ifdef PERF
			std::cout << " (biased by perf measurements)";
		    #endif
		    std::cout << "\n";
		}
	...
	//----------------------------------------------------------------
	    ...
	    Error(std::string("Unknown directive").c_str(), ANNabort);
	}
    }
    //--------------------------------------------------------------------
    ...
    PtType		type)		// point type
{
	...
	case PAR_LINES:				// uniform on 2 parallel lines
	    ...
	    break;
	default:
	    Error(std::string("INTERNAL ERROR: Unknown distribution").c_str(), ANNabort);
	    break;
    }

    if (stats > SILENT) {
	if(type == DATA) std::cout << "[Generating Data Points:\n";
	else             std::cout << "[Generating Query Points:\n";
    	std::cout << "  size          = " << n << "\n";
    	std::cout << "  dim           = " << dim << "\n";
    	std::cout << "  distribution  = " << distr_table[distr] << "\n";
	if (idum < 0)
	    std::cout << "  seed          = " << idum << "\n";
	if (distr == GAUSS || distr == CLUS_GAUSS)
    	    std::cout << "  std_dev       = " << std_dev << "\n";
	if (distr == CO_GAUSS || distr == CO_LAPLACE)
    	    std::cout << "  corr_coef     = " << corr_coef << "\n";
	if (distr == CLUS_GAUSS)
    	    std::cout << "  colors        = " << n_color << "\n";
    }
						// want to see points?
    if ((type == DATA  && stats >= SHOW_PTS) ||
	(type == QUERY && stats >= QUERY_RES)) {
	if(type == DATA) std::cout << "(Data Points:\n";
	else             std::cout << "(Query Points:\n";
	for (int i = 0; i < n; i++) {
	    std::cout << "    " << i << "\t";
	    printPoint(pa[i], dim);
	    std::cout << "\n";
    ...
    PtType		type)		// point type (DATA, QUERY)
{
    ...
    //--------------------------------------------------------------------
    std::ifstream in_file(file_nm);			// try to open data file
    if (!in_file)
	Error(std::string("Cannot open input data/query file").c_str(), ANNabort);
	...
	if (type == DATA) 
	    Error(std::string("`data_size' too small. Input file truncated.").c_str(), ANNwarn);
	else
	    Error(std::string("`query_size' too small. Input file truncated.").c_str(), ANNwarn);
    }
    ...
    if (stats > SILENT) {
    	if (type == DATA) {
	    std::cout << "[Read Data Points:\n";
	    std::cout << "  data_size  = " << n << "\n";
	}
    	else {
	    std::cout << "[Read Query Points:\n";
	    std::cout << "  query_size  = " << n << "\n";
	}
	std::cout << "  file_name  = " << file_nm << "\n";
    	std::cout << "  dim        = " << dim << "\n";
...
{
    if (stats > SILENT) {
	std::cout << "(Computing true nearest neighbors for validation.)\n";
    }
						// deallocate existing storage
...
void doValidation()			// perform validation
{
	...
	Error(std::string("Cannot validate with fewer true near neighbors than actual").c_str(), ANNabort);
    }
	...
	//--------------------------------------------------------------------
		...
		Error(std::string("INTERNAL ERROR: True nearest neighbor incorrect").c_str(), ANNabort);
	    }
		...
		Error(std::string("INTERNAL ERROR: Average error exceeds epsilon").c_str(), ANNabort);
	    }
	    #ifdef PERF
