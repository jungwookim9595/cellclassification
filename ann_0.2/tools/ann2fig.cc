...
{
    if (level == ANNabort) {
        std::cerr << "test: ERROR------->" << msg << "<-------------ERROR\n";
        exit(1);
    }
    else {
        std::cerr << "test: WARNING----->" << msg << "<-------------WARNING\n";
    }
}
...
void getArgs(int argc, char **argv)
{
	...
	if (!strcmp(argv[i], "-upi")) {		// process -upi option
		...
		Error(std::string("Slice dimension out of bounds").c_str(), ANNabort);
	    }
	...
	Error(std::string("File argument is required").c_str(), ANNabort);
    }

    ifile.open(infile_name, std::ios::in);		// open for reading
    if (!ifile) {
	Error(std::string("Cannot open input file").c_str(), ANNabort);
    }
    ofile.open(outfile_name, std::ios::out);		// open for writing
    if (!ofile) {
	Error(std::string("Cannot open output file").c_str(), ANNabort);
    }
...
void recReadTree(ANNorthRect &box)
{
	...
	if (lb != box.lo[cd] || hb != box.hi[cd]) {
	    Error(std::string("Bounding box coordinates are fishy").c_str(), ANNwarn);
	}
	...
	Error(std::string("Illegal node type in dump file").c_str(), ANNabort);
    }
}
...
void readANN()
{
	...
	Error(std::string("Incorrect header for dump file").c_str(), ANNabort);
    }
	...
	for (int i = 0; i < n_pts; i++) {	// input point coordinates
	    ...
	    if (idx < 0 || idx >= n_pts) {
		Error(std::string("Point index is out of range").c_str(), ANNabort);
	    }
	    for (j = 0; j < dim; j++) {
	...
	if (dim_x > dim || dim_y > dim) {
	    Error(std::string("Dimensions out of bounds").c_str(), ANNabort);
	}
	ifile >> n_pts;				// number of points
	...
	Error(std::string("Illegal ann format.  Expecting section heading").c_str(), ANNabort);
    }
}
